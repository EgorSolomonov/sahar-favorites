...
<div class="container">
	<h2 class="text-center">Товары со скидкой и по акции</h2>
<div class="row news-list<?=$themeClass?>">
		...
		<?endif;?>
			...
			<?foreach($arResult["ITEMS"] as $arItem):?>
				...
				?>
				<div class="news-list-item mb-2 <? if($arParams['NEWS_COUNT'] >= 3){echo("col-xl-4");} else{echo ("col-xl-6");}?> col-sm-12" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					<div class="card">
						<?if($arParams["DISPLAY_PICTURE"]!="N"):?>
							...
							else if ($arItem["SLIDER"])
							{
								...
								<div class="news-list-item-embed-img">
									...
									{
										?>
										<a href="<?	echo($arItem['PROPERTIES']['CATALOG_HREF']['~VALUE']['TEXT'])?>">
											<img
												class="card-img-top"
						...
						<div class="card-body">
							...
							<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
								<h4 class="card-title">
									<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
										<a href="<?	echo($arItem['PROPERTIES']['CATALOG_HREF']['~VALUE']['TEXT'])?>"><?echo $arItem["NAME"]?></a>
									<?else:?>
										<?echo $arItem["NAME"]?>
									<?endif;?>
								</h4>
