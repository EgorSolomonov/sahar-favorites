...
?>
<div class="bx_filter <?if ($arParams["FILTER_VIEW_MODE"] == "horizontal"):?>bx_horizontal<?endif?>">
            ...
            <div class="bx_filter_block">
                <div class="bx_filter_parameters_box_container" style="overflow: initial;">
                    ...
                    <div class="bx_ui_slider_track" id="drag_track_<?=$key?>">
                        ...
                        ?>
                        <div class="bx_ui_slider_part p1"><span><?echo round($price1);?></span></div>
                        ...
                        <div class="bx_ui_slider_part p5"><span><?echo round($price5);?></span></div>
	...
	switch ($arItem["DISPLAY_TYPE"])
	{
		...
		case "G"://CHECKBOXES_WITH_PICTURES
			...
			if ($ar["DISABLED"])
				$class.= " disabled";
			?>
			<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" title="<?=$ar["VALUE"]?>" class="bx_filter_param_label dib<?=$class?>" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
			...
			?>
                                <?foreach($arItem["VALUES"] as $val => $ar):?>
                                        <input
                                                                ...
                                                                onclick="smartFilter.click(this)"
                                                        />
                                        <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label checkbox <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
                                                <? echo $ar["VALUE"]; ?>
                                        </label>
                                <?endforeach;?>
			<?
...
<script>
	var smartFilter = new JCSmartFilter('<?echo CUtil::JSEscape($arResult["FORM_ACTION"])?>', '<?=CUtil::JSEscape(strtolower($arParams["FILTER_VIEW_MODE"]))?>', <?=CUtil::PhpToJSObject($arResult["JS_FILTER_PARAMS"])?>);
</script>