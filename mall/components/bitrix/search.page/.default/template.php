...
<form class="search_page_form" action="" method="get">
...
<?else:?>
	<input class="search_page_filed" type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" size="40" />
<?endif;?>
<?if($arParams["SHOW_WHERE"]):?>
	&nbsp;<select class="search_page_where" name="where">
...
<?endif;?>
	&nbsp;<input class="search_page_go" type="submit" value="<?=GetMessage("SEARCH_GO")?>" />
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<br />
	<div class="search_result"><?foreach($arResult["SEARCH"] as $arItem):?>
    <div class="search_page_line">

	    <?//  БЕРЁМ ID ТОВАРА ПО ID ТОРГОВОГО ПРЕДЛОЖЕНИЯ  ?>
	    <?$tovarID = CCatalogSku::GetProductInfo($arItem["ITEM_ID"])?>

	    <?//	Если элемент является товаром, а не торговым предложением, записывается его id в переменную для поиска картинок ?>
	    <?if(!$tovarID){
		    $tovarID['ID'] = $arItem["ITEM_ID"];
	    }?>

			<?//	БЕРЁМ КАРТИНКУ АНОНСА ТОВАРА ПО ID  ?>
	    <?$tovarPhoto = \Bitrix\Iblock\Elements\Elementcat1cTable::getList([
	    		'select' => ['PREVIEW_PICTURE'],
			    'filter' => ['=ID' => $tovarID['ID']]
	    ])->fetchAll();?>

	    <?//  РЕСАЙЗИМ ПОЛУЧЕНОЕ ФОТО ДО РАЗМЕРА 120Х120   ?>
	    <?$tovarPhotoURLsmall = CFile::ResizeImageGet($tovarPhoto[0]["PREVIEW_PICTURE"], array('width'=>120, 'height'=>120), BX_RESIZE_IMAGE_PROPORTIONAL);?>
	    <div class="center_photo">
		    <img src="<?=$tovarPhotoURLsmall["src"]?>" alt="<?=$arItem["TITLE_FORMATED"]?>">
