	...
	BX.Sale.OrderAjaxComponent = {
		...
		startLoader: function()
		{
			...
			if (!this.loadingScreen)
			{
				this.loadingScreen = new BX.PopupWindow("loading_screen", null, {
					overlay: {backgroundColor: 'white', opacity: '90'},
					events: {
						onAfterPopupShow: BX.proxy(function(){
							BX.cleanNode(this.loadingScreen.popupContainer);
							BX.removeClass(this.loadingScreen.popupContainer, 'popup-window');
							this.loadingScreen.popupContainer.appendChild(
								BX.create('IMG', {props: {src: "/local/templates/main/images/loader.svg"}})
							);
							...
							children: [
								BX.create('DIV', {
									...
									children: [
										...
										BX.create('DIV', {html: BX.util.htmlspecialcharsback(text)})
									]
								})
							]
						})
		...
		editCoupons: function(basketItemsNode)
		{
			var couponsList = this.getCouponsList(true),
				...
				couponsBlock = BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-coupon-input material-icons'},
							children: [
								BX.create('INPUT', {
									props: {
										id: 'coupon',
		...
		getCouponNode: function(coupon, active)
		{
			...
			return BX.create('STRONG', {
				...
				children: active ? [
					couponName || '',
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'bx-soa-coupon-remove material-icons'},
						events: {
							click: BX.proxy(function(e){
								var target = e.target || e.srcElement,
			...
			if (altProperty)
			{
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'form-control bx-soa-customer-input bx-ios-fix input-text',
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-sm-9 col-xs-10 bx-soa-pp-company-selected'},
						children: arNodes
					})
				);
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-sm-3 col-xs-2 bx-soa-pp-price'},
				...
				for (i = 0; i < activeStores.length; i++)
				{
					...
					geoObj = new ymaps.Placemark([activeStores[i].GPS_N, activeStores[i].GPS_S], {
						...
						balloonContentLayout: ymaps.templateLayoutFactory.createClass(
							...
							'<br /><a class="btn btn_blue" data-store="{{ properties.id }}">{{ properties.text }}</a>',
							{
								build: function() {
			...
			storeNode = BX.create('DIV', {
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {href: '', className: 'btn btn_blue'},
								text: this.params.MESS_SELECT_PICKUP,
								events: {
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (settings.IS_PHONE == 'Y') {
					textNode.setAttribute('autocomplete', 'tel');
                                        setTimeout(function(){$("#"+textNode.id).mask("+7 (999) 999-99-99");}, 100);
                                }
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'form-control bx-soa-customer-input bx-ios-fix input-text');
			}
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.proxy(function(e){
					...
					if (textInputs.length)
					{
						...
						BX.addClass(textInputs[textInputs.length - 1], 'form-control bx-soa-customer-input bx-ios-fix input-text');
		...
		editTotalBlock: function()
		{
            ...
            {
                var price_sbor = total.DISCOUNT_PRICE_FORMATED.replace(/\-/gi,'');
                this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_WEIGHT_SUM_SBOR'), price_sbor));
            }
		...
		totalBlockScrollCheck: function()
		{
			...
			var scrollTop = BX.GetWindowScrollPos().scrollTop,
				ghostTop = BX.pos(this.totalGhostBlockNode).top - (header_height + 10),
