	...
	if (!empty($arResult['SKU_PROPS']))
	{
		foreach ($arResult['SKU_PROPS'] as $arProp)
		{
			...
			if ('TEXT' == $arProp['SHOW_MODE'])
			{
				...
				$skuTemplate[$propId]['FULL']['START'] = /* '<span class="scu_name">'.htmlspecialcharsbx($arProp['NAME']).':</span> - для размера */'<ul>';
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	<?dump($arItem['PROPERTIES']["OSEN_ZIMA"]["VALUE"])?>
	<?dump($arItem['PROPERTIES']["VESNA_LETO"]["VALUE"])?>
		...
		<?endif;?>
		<? if($arItem['PROPERTIES']["POSLEDNIY_RAZMER"]["VALUE"] == 'Да'):?>
		...
		<?// echo 'ОСЕНЬ - ЗИМА - '.$arItem['PROPERTIES']['OSEN_ZIMA']["VALUE"];?>
		<?// echo 'ЗИМА - '.$arItem['PROPERTIES']['SEZON_ZIMA']["VALUE"];?>
		<?// echo 'ВЕСНА - ЛЕТО - '.$arItem['PROPERTIES']['VESNA_LETO']["VALUE"];?>
		<?// echo 'ЛЕТО - '.$arItem['PROPERTIES']['SEZON_LETO']["VALUE"];?>
		<?echo 'НОВЫЙ ГОД - '.$arItem['PROPERTIES']['NOVYY_GOD']["VALUE"];?>
		
		<?// echo 'NOVINKA - '.$arItem['PROPERTIES']['NOVINKA']["VALUE"];?>

		<?// echo 'LIDER_PRODAZH - '.$arItem['PROPERTIES']['LIDER_PRODAZH']["VALUE"];?>
		<?// echo 'KHODOVOY_TOVAR - '.$arItem['PROPERTIES']['KHODOVOY_TOVAR']["VALUE"];?>
		<?// echo 'ROZNICHNYY_INTERNET_MAGAZIN - '.$arItem['PROPERTIES']['ROZNICHNYY_INTERNET_MAGAZIN']["VALUE"];?>
		<?// echo 'RASPRODAZHA - '.$arItem['PROPERTIES']['RASPRODAZHA']["VALUE"];?>
		<?// echo 'NOVYY_GOD - '.$arItem['PROPERTIES']['NOVYY_GOD']["VALUE"];?>
	...
	if (!empty($minPrice))
	{
                if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])
		{
			...
			$str = "от";
                    ...
                    ?>
                    <span class="old_price"><?//=$str?> 11<? echo $minPrice['PRINT_VALUE']; ?></span>
                    <?
		}
		if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		{
?>
<!-- 			<div class="bx_catalog_item_articul"> - раскомментить - если нужен будет артикул
<?
