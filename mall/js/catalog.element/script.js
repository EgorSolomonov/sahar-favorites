...
window.JCCatalogElement.prototype.SelectOfferProp = function()
{
    ...
    $(".one-click-btn", this.obProduct).attr("data-offer-id", this.offers[this.offerNum].ID);
    
    if(offers_url_change) {
...
window.JCCatalogElement.prototype.SetCurrent = function()
{
	var i = 0,
		...
		current = this.offers[this.offerNum].TREE,
                current_offer_id = this.offers[this.offerNum].ID;
	  ...
	  for (i = 0; i < this.offers.length; i++)
	  {
	     if (this.offers[i].ID === pid[1]) {
	        current = this.offers[i].TREE;
                current_offer_id = this.offers[i].ID;
	     }
	  }
	}
        
        $(".one-click-btn", this.obProduct).attr("data-offer-id", current_offer_id);
        //CUSTOM END
...
window.JCCatalogElement.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		if(this.offers[index].AVAILABILITY) {
            BX.adjust(BX(this.visual.ID+"_sku_available"),{ html: this.offers[index].AVAILABILITY});
		}
...
{
        if(!$(this.obAddToBasketBtn).hasClass("loading")) {
            $(this.obAddToBasketBtn).addClass("loading");
            this.basketMode = 'ADD';
            this.Basket();
        }
};
